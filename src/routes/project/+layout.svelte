...
<div class="grid h-full md:flex">
	<Wrapper wrapperClass="md:border-r border-b border-gray-800 md:h-[calc(100dvh-8rem)] h-auto md:min-w-[340px] md:max-w-[340px]" header="Project">
		<p>
		...
		<div class="not-prose space-y-4">
			<div class="grid divide-y divide-gray-700 first:border-t first:border-gray-700 last:border-b last:border-gray-700">
				{#each links as link}
					...
					<a href="/project/{link.url}" class="group flex items-center justify-between py-4">
						<div class="flex w-full items-center gap-4">
							<div
								class="relative flex shrink-0 items-center justify-center overflow-hidden rounded-full bg-accent-600 text-sm transition-all duration-300 {isCurrent
									? 'h-6 w-16'
									: 'h-2 w-8 group-hover:h-6 group-hover:w-16'}"
							>
						...
						<Icon
							...
							class="shrink-0 text-2xl transition-all duration-300 md:-rotate-90 {isCurrent ? 'scale-100' : 'scale-0 group-hover:scale-100'}"
						/>
					</a>
				{/each}
			</div>
